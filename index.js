const {
  ...
  if (!fs.existsSync(path.join(__dirname, 'sessions', 'creds.json'))) {
      ...
      try {
          ...
          filer.download((err, data) => {
              ...
              fs.writeFile(path.join(__dirname, 'sessions', 'creds.json'), data, (writeErr) => {
                  if (writeErr) {
  ...
  async function connectToWA() {
      ...
      try {
          ...
          // Wait a bit if sessions directory was just created
...
╰─🚀 *Powered by Alex Macksyn*`;
          ...
          // Add all the helper functions from your original code
          conn.decodeJid = jid => {
              if (!jid) return jid;
              if (/:\d+@/gi.test(jid)) {
